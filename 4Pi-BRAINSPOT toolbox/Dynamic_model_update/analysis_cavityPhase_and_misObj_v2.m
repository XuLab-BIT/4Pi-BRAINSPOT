%  Script for cavity phase and objective misalignment estimation
...
%       
%%
function sobj_phi0 = analysis_cavityPhase_and_misObj_v2(cavity, tform_all, setup_para)
...
for nn = 1:cavity.dirN
    ...
    if nn <= cavity.loopn
        [phi0_guess_keep,phi0_fit,misobj_guess_keep,misobj_fit] = caliphi0_and_misObj_NCC_v3(subregion_chs,cavity.probj_all{step_num});
    else
        [phi0_guess_keep,phi0_fit,misobj_guess_keep,misobj_fit] = caliphi0_and_misObj_NCC_v3(subregion_chs,cavity.probj_all{step_num},phi0_fit_cycle(nn-loopn),misobj_fit_cycle(nn-loopn,:));     
    end

    phi0_keep_cycle{nn} = phi0_guess_keep;
    phi0_fit_cycle = [phi0_fit_cycle; phi0_fit];
    misobj_keep_cycle{nn} = misobj_guess_keep;
    misobj_fit_cycle = [misobj_fit_cycle; misobj_fit];
    
end

sobj_phi0 = struct('phi0_keep_cycle',phi0_keep_cycle,'phi0_fit_cycle',phi0_fit_cycle,'misobj_keep_cycle',misobj_keep_cycle,'misobj_fit_cycle',misobj_fit_cycle);
save(fullfile(setup.workspace,'phi0_misObj_est'), 'sobj_phi0');
