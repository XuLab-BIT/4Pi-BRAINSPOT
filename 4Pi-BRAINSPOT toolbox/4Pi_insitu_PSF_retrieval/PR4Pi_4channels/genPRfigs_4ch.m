function genPRfigs_4ch(obj,ImgType)
...
%                 imagesc(Mpsf(Ro/2-Ri/2+1:Ro/2+Ri/2,Ro/2-Ri/2+1:Ro/2+Ri/2,zind(jj)),[min_color max_color]);
        ...
        for ii = 1:N_ch
            Modpsf = squeeze(obj.PSFstruct.Modpsf(:,:,:,ii));  
            for jj = 1:L
                ha = axes('position',[(jj-1)/L,(N_ch-ii)/N_ch,1/L,1/N_ch],'parent',h1);
...
%                 imagesc(Modpsf(Ro/2-Ri/2+1:Ro/2+Ri/2,Ro/2-Ri/2+1:Ro/2+Ri/2,zind(jj)),[min_color max_color]);
    ...
    case 'pupil'
        ...
        axis(h2,'off')
    case 'zernike'
        PlotZernikeC(obj.PRstruct1.Zernike_phase,'phase A');   
        PlotZernikeC(obj.PRstruct1.Zernike_mag,'magnitude A');
        PlotZernikeC(obj.PRstruct2.Zernike_phase,'phase B');   
        PlotZernikeC(obj.PRstruct2.Zernike_mag,'magnitude B');
